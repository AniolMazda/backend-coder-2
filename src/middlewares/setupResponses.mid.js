const setupResponses = (req,res,next) => {
    try {
        ...
        const sucessResponse = (code,response,message=messages[code]) => res.status(code).json({method,url,response,message})
        const errorResponse = (code, errorMessage=messages[code]) => {
            const error = new Error(errorMessage)
            error.statusCode = code
            throw error
        }
        res.json200 = (response,message) => sucessResponse(200, response, message)
        res.json201 = (response,message) => sucessResponse(201, response, message)
